...
/** True if the cell is made entirely of asterisk / dash separator characters. */
function isSeparatorCell(value: unknown): boolean {
  const text = normalizeHeaderText(value);
  if (!text) return false;
  return /^[*\-\s=_.]+$/.test(text);
}

/** True if the cell looks like a footer label (e.g. "HDFC Bank GSTIN number…"). */
function isFooterLabelCell(value: unknown): boolean {
  const text = normalizeHeaderText(value).toLowerCase();
  if (!text) return false;
  // A footer label is a multi-word phrase whose first token isn't a digit and
  // which doesn't look like any common bank narration prefix.
  if (/^\d/.test(text)) return false;
  if (text.length < 5) return false;
  // Quick reject for known good prefixes.
  if (
    /^(upi|pos|dc|ach|neft|imps|rtgs|nft|byi|sby|inf|nfs|card|cr|dr)/i.test(
      text,
    )
  ) {
    return false;
  }
  return /(gstin|gstn|registered office|state acc|branch code|generated|http|www\.|senapati|bapat|end of statement|---)/i.test(
    text,
  );
}
...
export function parseStatementBuffer(buffer: ArrayBuffer): StatementParseResult {
  ...
  for (let i = headerIdx + 1; i < rows.length; i++) {
    ...
    // Silently skip rows whose date cell is clearly not a date — these are
    // separator / footer rows (e.g. "********", "HDFC Bank GSTIN…").
    if (isSeparatorCell(rawDate) || isFooterLabelCell(rawDate)) continue;
